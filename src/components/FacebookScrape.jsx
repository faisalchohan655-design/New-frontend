...
const FacebookScrape = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const toastId = toast.loading('Scraping Facebook...');
    try {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent mb-6">Facebook Page Scraper</h1>
      <div className="bg-white rounded-2xl shadow-xl p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div><label className="block text-sm font-medium text-gray-700"><FaFacebook className="inline mr-1 text-blue-600"/> Facebook Page URL</label>
          ...
          <div className="border-t pt-4">
            <h3 className="font-semibold text-gray-700 mb-3">Quality Filters</h3>
          ...
          <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-3 rounded-xl flex items-center justify-center gap-2">{loading ? '⏳' : <FaSearch/>} {loading ? 'Scraping...' : 'Scrape Facebook Page'}</button>
        </form>
      </div>
    </div>
  );
