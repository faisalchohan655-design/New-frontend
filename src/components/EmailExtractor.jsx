...
const EmailExtractor = () => {
  ...
  const saveToLeads = async () => {
    ...
    try {
      ...
      toast.success(`Saved ${filteredLeads.length} leads to database`);
    } catch (err) {
  ...
  const sendEmails = async () => {
    ...
    try {
      ...
      toast.success(`Sent to ${selectedEmails.length} recipients`);
      ...
      {extractedLeads.length > 0 && (
        <div className="bg-white rounded-xl shadow p-4 mb-6">
          <div className="flex flex-wrap gap-4 items-center">
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => { setFilterVerified(e.target.checked); setTimeout(applyFilters, 0); }}
              />
              Verified only
            </label>
            <input
              ...
              value={filterDomain}
              onChange={(e) => { setFilterDomain(e.target.value); setTimeout(applyFilters, 0); }}
          ...
          <div className="mt-3 text-sm text-gray-500">
            {filteredLeads.length} emails shown (total {extractedLeads.length})
          </div>
        </div>
      )}
            ...
            <tbody>
              {filteredLeads.map((lead, idx) => (
                <tr key={idx} className="border-t">
                  <td className="p-3"><input type="checkbox" checked={selectedEmails.includes(lead.email)} onChange={() => toggleSelect(lead.email)} /></td>
                  <td className="p-3">{lead.email}</td>
                  <td className="p-3">{lead.verified ? '✅ Yes' : '❌ No'}</td>
                  <td className="p-3">{lead.phone || '-'}</td>
                  <td className="p-3 truncate max-w-xs">{lead.source}</td>
                </tr>
              ))}
            </tbody>
          </table>
