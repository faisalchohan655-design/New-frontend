...
const EmailExtractor = () => {
  ...
  const exportExcel = () => {
    ...
    const data = extracted.map(e => ({ Email: e.email, Source: e.source, Verified: e.verified ? 'Yes' : 'No' }));
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6">
        ...
        {extracted.length > 0 && (
          <div className="mt-6">
            ...
            <div className="mt-3 text-sm">Found {extracted.length} emails</div>
            <ul className="max-h-64 overflow-auto">
              {extracted.map((e, i) => <li key={i}>{e.email} {e.verified ? '✅' : ''} <span className="text-xs text-gray-400">{e.source}</span></li>)}
            </ul>
          </div>
        )}
      </div>
