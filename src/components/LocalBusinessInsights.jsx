        ...
        <form onSubmit={handleScrape} className="space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={keyword} onChange={e => setKeyword(e.target.value)} className="w-full border rounded-xl p-2" required />
            </div>
            <div>
              <label className="block text-sm font-medium">City / Location</label>
              <input type="text" value={city} onChange={e => setCity(e.target.value)} className="w-full border rounded-xl p-2" required />
            </div>
          </div>
          <div>
          ...
          <div className="bg-white p-4 rounded-xl shadow mb-4">
            <div className="flex flex-wrap gap-4 items-end">
              ...
              <div>
                ...
                <input type="text" placeholder="e.g., Karachi" value={filters.cityFilter} onChange={e => setFilters({ ...filters, cityFilter: e.target.value })} className="border rounded p-1" />
              </div>
              <div>
              ...
              <tbody>
                {filteredLeads.map(lead => (
                  <tr key={lead._id} className="border-t">
                    ...
                    <td className="p-3">{lead.website ? <a href={lead.website} target="_blank" className="text-blue-600">Visit</a> : '-'}</td>
                    <td className="p-3 max-w-xs truncate">{lead.address || '-'}</td>
                    <td className="p-3">{lead.rating || '-'}</td>
                    <td className="p-3"><button onClick={() => handleDelete(lead._id)} className="text-red-500">🗑️</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
